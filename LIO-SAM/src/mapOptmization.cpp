...
public:
    ...
    // 创建一个共享指针，指向tf2_ros::Buffer对象。tf2_ros::Buffer用于存储和操作坐标变换信息。
    std::shared_ptr<tf2_ros::Buffer> tfBuffer;

    // 创建一个共享指针，指向tf2_ros::TransformBroadcaster对象。tf2_ros::TransformBroadcaster用于发布坐标变换信息到TF2系统。
    std::shared_ptr<tf2_ros::TransformBroadcaster> tfBroadcaster;

    // 创建一个共享指针，指向tf2_ros::TransformListener对象。tf2_ros::TransformListener用于从TF2系统中监听和获取坐标变换信息。
    ...
    void laserCloudInfoHandler(const lio_sam::msg::CloudInfo::SharedPtr msgIn)
    {
        ...
        // using _cloud_corner_type = sensor_msgs::msg::PointCloud2_<std::allocator<void>>;
        // _cloud_corner_type cloud_corner;
        // pcl::fromROSMsg(cloud_corner,  *laserCloudCornerLast);
